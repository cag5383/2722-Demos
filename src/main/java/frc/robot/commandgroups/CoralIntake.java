// Copyright (c) FIRST and other WPILib contributors.
// Open Source Software; you can modify and/or share it under the terms of
// the WPILib BSD license file in the root directory of this project.

package frc.robot.commandgroups;

import edu.wpi.first.wpilibj2.command.SequentialCommandGroup;
import frc.robot.subsystems.Elevator.Elevator;
import frc.robot.subsystems.Intake.*;
import frc.robot.subsystems.Wrist.*;
import frc.robot.commands.ConditionalSetWristPosition;
import frc.robot.commands.RunIntakeMotor;
// NOTE:  Consider using this command inline, rather than writing a subclass.  For more
// information, see:
// https://docs.wpilib.org/en/stable/docs/software/commandbased/convenience-features.html
public class CoralIntake extends SequentialCommandGroup {
  /** Creates a new CoralIntake. */
  private final Elevator m_elevator;
  private final Wrist m_wrist;
  private final Intake m_intake; 

  public CoralIntake(Elevator elevatorInput, Wrist wristInput, Intake intakeInput) {
    // Add your commands in the addCommands() call, e.g.
    // addCommands(new FooCommand(), new BarCommand());
    m_elevator = elevatorInput;
    m_wrist = wristInput;
    m_intake = intakeInput;

    // addCommands(new ConditionalSetWristPosition(m_wrist, WristConstants.kWristElevatorSafeSetpoint));
    // addCommands(new ConditionalSetElevatorPosition(m_elevator, ElevatorConstants.kElevatorIntakeSetpoint));
    // addCommands(new RunElevatorMotorsSoftLimitVersion(m_elevator, -0.1));
    addCommands(new ElevatorSmoothDown(m_elevator));
    addCommands(new ConditionalSetWristPosition(m_wrist, WristConstants.kWristIntakeSetpoint));
    addCommands(new RunIntakeMotor(m_intake, IntakeConstants.kCoralIntakeSpeed));
  }
}
